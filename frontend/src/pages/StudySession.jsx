...
import {
  ...
  PenLine, GraduationCap, Zap, Delete,
} from 'lucide-react'
    ...
    <div className="space-y-5 animate-fade-in">
      <div className="card text-center py-8 space-y-4">
        ...
        <img
          ...
          alt=""
          className="mx-auto w-48 h-36 object-cover rounded-xl border border-dark-400"
          onError={(e) => { e.target.src = DEFAULT_IMG }}
        />
        <p className="text-4xl font-bold text-slate-100 heb">{word.hebWord}</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {word.stage1Options.map((opt, i) => (
          <button
            ...
            className={`py-4 px-5 rounded-xl border-2 text-center font-medium transition-all duration-200 ${optionCls(
              answered,
              opt === word.engWord,
              opt === selectedOption
            )}`}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {word.stage2Options.map((opt, i) => (
          <button
            ...
            className={`py-4 px-5 rounded-xl border-2 text-center font-medium heb transition-all duration-200 ${optionCls(
              answered,
              opt === word.hebWord,
              opt === selectedOption
            )}`}
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <div className="card text-center py-8 space-y-3">
        <p className="text-xs text-slate-500 uppercase tracking-wider">Listen and choose the correct Hebrew word</p>
        <button
          onClick={() => speak(word.engWord, 'en-US')}
          className="mx-auto flex flex-col items-center gap-2 group"
        >
          <div className="w-16 h-16 rounded-full bg-primary/15 border border-primary/30 flex items-center justify-center group-hover:bg-primary/25 transition-colors">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {word.stage3Options.map((opt, i) => (
          <button
            ...
            className={`p-4 rounded-xl border-2 flex flex-col items-center gap-3 transition-all duration-200 ${optionCls(
              answered,
              opt.isCorrect,
              opt.hebWord === selectedOption
            )}`}
          >
            <img
              src={getImg(opt.image_url)}
              alt=""
              className="w-24 h-20 object-cover rounded-lg border border-dark-400/50"
              onError={(e) => { e.target.src = DEFAULT_IMG }}
            />
            <span className="heb font-medium text-center">{opt.hebWord}</span>
          </button>
        ))}
      </div>
    </div>
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="card text-center py-8 space-y-4">
        <p className="text-xs text-slate-500 uppercase tracking-wider">
          Spell the English word · {letterCount} letters
        </p>
        <img
          src={getImg(word.image_url)}
          alt=""
      ...
      {/* Built word display */}
      <div className="min-h-16 flex items-center justify-center gap-2 flex-wrap px-2">
      ...
      {submitted && !isComplete && (
        <p className="text-center text-slate-400 text-sm">
          Correct spelling: <span className="text-emerald-400 font-bold">{word.engWord}</span>
        </p>
      )}

      {/* Letter bank */}
      <div className="card">
        <p className="text-xs text-slate-600 mb-3 text-center uppercase tracking-wider">Letter Bank</p>
...
function StageTransition({ completedStage, nextStage, wordsCount, onContinue }) {
  ...
  return (
    <div className="max-w-md mx-auto text-center space-y-6 animate-fade-in">
      <div className="card py-12 space-y-6">
        ...
        <div>
          <h2 className="text-2xl font-bold text-slate-100">Stage {completedStage} Complete!</h2>
          <p className="text-slate-500 text-sm mt-1">Practiced all {wordsCount} words</p>
        </div>

        {next && (
          <div className={`rounded-xl border p-4 ${next.bg} ${next.border}`}>
            <div className="flex items-center justify-center gap-2 mb-1">
              {NextIcon && <NextIcon size={18} className={next.color} />}
        ...
        <button
          className="btn-primary px-8 py-2.5 flex items-center gap-2 mx-auto"
          onClick={onContinue}
        >
          {nextStage
            ? <>Continue to Stage {nextStage} <ArrowRight size={16} /></>
      ...
      <div className="flex items-center justify-center gap-2">
        {[1, 2, 3, 4].map((s) => (
          <div key={s} className="flex items-center">
            ...
            {s < 4 && (
              <div
                className={`w-8 h-0.5 mx-1 ${s < completedStage ? 'bg-emerald-500/40' : 'bg-dark-400'}`}
              />
            )}
          </div>
        ))}
    ...
    <div className="max-w-3xl mx-auto space-y-5 animate-fade-in">
      ...
      <div className="card divide-y divide-dark-400/50 p-0 overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-14 text-slate-500 text-sm">No words found</div>
        ) : (
          allWords.map((word) => {
            ...
            return (
              ...
              >
                ...
                {word.image_url && (
                  <div className="w-8 h-8 rounded-md overflow-hidden flex-shrink-0 border border-dark-400">
                    <img
...
function SessionResults({ score, mistakes, totalWords, onRetry, onNew }) {
  ...
  // Deduplicate mistakes by word+stage
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5 animate-fade-in">
      ...
      <div className="card text-center space-y-4 py-10">
        <Trophy size={48} className={`mx-auto ${grade.color}`} />
        ...
        <p className="text-slate-400 text-sm">
          ...
          &nbsp;·&nbsp;{totalWords} words × 4 stages
        </p>
        <div className="flex gap-3 pt-2 max-w-xs mx-auto">
          <button className="btn-ghost flex-1 py-2.5" onClick={onNew}>
            New Session
          </button>
      ...
      {uniqueMistakes.length > 0 && (
        <div className="card">
          <h3 className="text-sm font-semibold text-slate-300 mb-4">
            Words to Review ({uniqueMistakes.length})
          </h3>
          <div className="space-y-0 divide-y divide-dark-400/50">
            {uniqueMistakes.map((m, i) => (
              <div key={i} className="flex items-start gap-3 py-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-slate-400 text-sm heb mt-0.5">{m.word.hebWord}</p>
                </div>
                {m.word.image_url && (
                  <img
...
export default function StudySession() {
  const [phase, setPhase] = useState('select')       // select | session | transition | results
  ...
  // ── Select phase ──────────────────────────────────────────────────────────
  if (phase === 'select') {
    return <WordSelector onStart={startSession} />
  }
  ...
  // What to show as "correct answer" in feedback
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2">
            {StageIcon && <StageIcon size={15} className={stageMeta?.color} />}
            <span className="font-bold text-slate-100">{stageMeta?.title}</span>
            <span className="text-slate-600 text-sm hidden sm:inline">· {stageMeta?.subtitle}</span>
          </div>
          <p className="text-slate-500 text-sm mt-0.5">
            Word {wordIdx + 1}/{preparedWords.length} · {score.correct} correct
          </p>
        </div>
            ...
            {isCorrect ? (
              ...
              <>
                ...
                <span className="text-red-400 font-medium">
                  Answer:{' '}
                  <span className={hintIsHeb ? 'heb' : ''}>{correctAnswerHint}</span>
                </span>
              </>
            )}
          </div>
